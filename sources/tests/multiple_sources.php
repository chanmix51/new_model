...
class TikOwnerTable implements SqlSource
{
    ...
    {
        return "multiple_source_test.tik_owner";
    }
}
...
class TikTable implements SqlSource 
{
    ...
    public function getDefinition(): string
    {
        return "multiple_source_test.tik";
    }
}
...
class TikOwnerProjectionMap implements ProjectionMap {
    ...
    public function __construct() {
        $this->projection = ProjectionFieldDefinition::fromStructure(source_name: "tik_owner", structure: (new TikOwnerTable)->getStructure());
        $this->addField(source_name: "tik", definition: "count(**)", field_name: 'nb_tik', type: 'int')
             ->addField(source_name: "tik", definition: "sum(**.tik_value)", field_name: "tik_value_sum", type: 'int')
             ->addField(source_name: "tik", definition: "avg(**.tik_value)", field_name: "mean_tik_value", type: 'float')
             ;
    }
...
// setup
$setup_sql = [
    "drop schema if exists multiple_source_test cascade",
    "create schema multiple_source_test",
    "create table multiple_source_test.tik_owner (tik_owner_id serial primary key, name text not null, created_at timestamptz not null default now())",
    "insert into multiple_source_test.tik_owner (name) values ('pika'), ('chu')",
    "create table multiple_source_test.tik as select tik_id as tik_id, floor(random() * 2)::int + 1 as tik_owner_id, floor(random() * 100)::int as tik_value from generate_series(1, 100) as pika(tik_id);",
    "alter table multiple_source_test.tik add foreign key (tik_owner_id) references multiple_source_test.tik_owner (tik_owner_id)",
];
